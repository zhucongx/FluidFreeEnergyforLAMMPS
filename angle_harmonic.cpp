...
void AngleHarmonic::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nanglelist; n++) {
    ...
    f1[0] = (a11 * delx1 + a12 * delx2) * fscale[type];
    f1[1] = (a11 * dely1 + a12 * dely2) * fscale[type];
    f1[2] = (a11 * delz1 + a12 * delz2) * fscale[type];
    f3[0] = (a22 * delx2 + a12 * delx1) * fscale[type];
    f3[1] = (a22 * dely2 + a12 * dely1) * fscale[type];
    f3[2] = (a22 * delz2 + a12 * delz1) * fscale[type];
...
void AngleHarmonic::allocate()
{
  ...
  memory->create(fscale,n+1,"angle:fscale");
