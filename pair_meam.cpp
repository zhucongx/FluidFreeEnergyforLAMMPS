...
/* ----------------------------------------------------------------------
   ...
   fscale variable (which multiplies only the forces) and the extract() 
   method were added by Rodolfo Paula Leite (Unicamp/BR).
------------------------------------------------------------------------- */
...
void PairMEAM::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum_half; ii++) {
    ...
    meam_inst->meam_force(i, eflag_global, eflag_atom, vflag_global, vflag_atom, &eng_vdwl, eatom,
                          ...
                          numneigh_full[i], firstneigh_full[i], offset, f, vptr, virial, fscale);
    offset += numneigh_half[i];
  }
...
void PairMEAM::allocate()
{
  ...
  memory->create(fscale, np1, np1, "pair:fscale");
  map = new int[np1];
}
