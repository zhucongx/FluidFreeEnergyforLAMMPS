...
{
  if ((narg < 1) || (narg > 2)) error->all(FLERR,"Illegal kspace_style {} command", force->kspace_style);
  if (narg == 2) fscale = fabs(force->numeric(FLERR,arg[1]));
  else fscale = 1.0;
...
void PPPM::compute(int eflag, int vflag)
{
  ...
  if (vflag_global) {
    ...
    for (i = 0; i < 6; i++) virial[i] = 0.5*qscale*fscale*volume*virial_all[i];
  }
  ...
  if (evflag_atom) {
    ...
    if (vflag_atom) {
      for (i = 0; i < ntotal; i++)
        for (j = 0; j < 6; j++) vatom[i][j] *= 0.5*qscale*fscale;
    }
  }
...
void PPPM::slabcorr()
{
  ...
  double ffact = qscale * (-4.0*MY_PI/volume) * fscale;
  double **f = atom->f;
...
void PPPM::compute_group_group(int groupbit_A, int groupbit_B, int AA_flag)
{
  ...
  f2group[0] = qscale*volume*f2group_all[0]*fscale;
  f2group[1] = qscale*volume*f2group_all[1]*fscale;
  if (slabflag != 2) f2group[2] = qscale*volume*f2group_all[2]*fscale;
...
void PPPM::slabcorr_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  ...
  const double ffact = qscale * (-4.0*MY_PI/volume) * fscale;
